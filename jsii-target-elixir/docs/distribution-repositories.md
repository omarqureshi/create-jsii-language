...
It generates Elixir for *any* jsii assembly and takes whatever naming it is
...
that library, not about Elixir. Those facts live in a separate repository per
...
| knows about | jsii, Elixir | one library, and this target as a dependency |
| example | `jsii-target-elixir` | `aws-cdk-elixir`, `cdk8s-elixir` |
...
```
config/profile.json      what this library is called in Elixir
...
assembly declares. Published assemblies carry no `targets.elixir` — external
...
```js
const { profileHarness } = require('jsii-target-elixir/testing');
...
- [`cdk8s-ruby`](https://github.com/omarqureshi/cdk8s-ruby) — smaller, and
  carries a conformance check that synthesizes the same chart from Elixir and
