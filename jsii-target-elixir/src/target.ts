...
/**
 * TODO(elixir): the file extension your language uses for source files.
 */
export const SOURCE_FILE_EXTENSION = '.elixir';

export class ElixirTarget extends Target {
  protected readonly generator: ElixirGenerator;

  public constructor(options: TargetOptions) {
    super(options);
    // Out-of-band naming (JSII_ELIXIR_TARGET_CONFIG) merges into the
    // assembly spec before anything reads targets.elixir from it.
    applyTargetOverlay(options.assembly.spec);
    this.generator = new ElixirGenerator(options.rosetta, options);
  }

  /**
   * Turns the generated sources into a distributable package.
   *
   * TODO(elixir): invoke your ecosystem's package build here (the Ruby
 ...
 * declaration order; this class turns them into Elixir source text via
 ...
 * job is to replace manifest entries with Elixir source, hook by hook.
 * Record each naming/ergonomics decision in docs/decisions.md as you go.
 */
export class ElixirGenerator extends Generator {
  ...
  protected override getAssemblyOutputDir(mod: spec.Assembly): string {
    ...
    // root. TODO(elixir): match your package layout (Ruby embeds it under
    // lib/<gem-name>/data/).
    return `lib/${mod.name}`;
  }
  ...
  protected override onEndAssembly(assm: spec.Assembly, _fingerprint: boolean): void {
    // TODO(elixir): when you start emitting real code, this manifest is
  ...
  protected override onBeginClass(cls: spec.ClassType, _abstract: boolean | undefined): void {
    // TODO(elixir): open the class declaration: name mapping
  ...
  protected override onEndClass(_cls: spec.ClassType): void {
    // TODO(elixir): close the class declaration.
  }

  protected onMethod(cls: spec.ClassType, method: spec.Method): void {
    // TODO(elixir): instance method: member naming
  ...
  protected onMethodOverload(cls: spec.ClassType, overload: spec.Method, _originalMethod: spec.Method): void {
    // TODO(elixir): languages with default/keyword arguments usually need
  ...
  protected onProperty(cls: spec.ClassType, prop: spec.Property): void {
    // TODO(elixir): getter (and setter unless prop.immutable) backed by
    // kernel get/set.
  ...
  protected onStaticProperty(cls: spec.ClassType, prop: spec.Property): void {
    // TODO(elixir): static property / constant.
  ...
  protected onUnionProperty(cls: spec.ClassType, prop: spec.Property, _union: spec.UnionTypeReference): void {
    // TODO(elixir): union-typed property — dynamically-typed languages
  ...
  protected onBeginInterface(ifc: spec.InterfaceType): void {
    // TODO(elixir): behavioral interfaces (ifc.datatype falsy) become your
  ...
  protected onEndInterface(_ifc: spec.InterfaceType): void {
    // TODO(elixir): close the interface declaration.
  }
  ...
  protected override onBeginEnum(enm: spec.EnumType): void {
    // TODO(elixir): docs/decisions.md#enums.
