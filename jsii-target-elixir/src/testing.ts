...
import { ElixirVisitor } from './rosetta/visitor';
import { loadTargetOverlay, TARGET_CONFIG_ENV } from './target-config';

/**
 * A harness for testing a naming profile.
 *
 * A profile — what a library is called in Elixir — is data this target knows
...
export interface ProfileHarness {
  /** The Elixir module path a jsii fqn is generated as under this profile. */
...
export interface ProfileHarnessOptions {
  /** Path to the profile JSON (the file `ELIXIR_TARGET_CONFIG` names). */
