...
import { ElixirVisitor } from '../src/rosetta/visitor';
...
// The snippets stay upstream — this repo contributes only .elixir
...
const EXPECTATION_EXT = '.elixir'; // TODO(elixir): your source extension
  ...
  describe('translations corpus', () => {
    ...
    for (const name of TranslationsCorpus.snippetNames()) {
      ...
      test(`translates ${name}.ts`, options, () => {
        ...
        const actual = normalizeRenderedSource(corpus.render(snippet, new ElixirVisitor()));
