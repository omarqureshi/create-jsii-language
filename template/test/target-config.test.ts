...
function withOverlay(overlay: unknown): void {
  const file = path.join(fs.mkdtempSync(path.join(os.tmpdir(), '{{lang}}-target-config-')), 'overlay.json');
...
describe('applyTargetOverlay', () => {
  ...
  test('merges root config, keeping other languages', () => {
    ...
    const assembly = fakeAssembly();
    applyTargetOverlay(assembly);
    assert.deepEqual(assembly.targets['{{lang}}'], { module: 'RootModule' });
  ...
  test('overlay wins over in-assembly config; unmentioned keys survive', () => {
    withOverlay({ 'aws-cdk-lib': { module: 'RootModule' } });
    const assembly = fakeAssembly();
    assembly.targets['{{lang}}'] = { module: 'WrongName', extra: 'kept' };
    applyTargetOverlay(assembly);
    assert.equal(assembly.targets['{{lang}}'].module, 'RootModule');
    assert.equal(assembly.targets['{{lang}}'].extra, 'kept');
  });
  ...
  test('merges into the dependency closure', () => {
    ...
    const assembly = fakeAssembly();
    applyTargetOverlay(assembly);
    assert.deepEqual(assembly.dependencyClosure.constructs.targets['{{lang}}'], { module: 'Constructs' });
  });
  ...
  test('merges submodule entries; tolerates unknown submodules', () => {
    ...
    const assembly = fakeAssembly();
    applyTargetOverlay(assembly);
    assert.equal(assembly.submodules['aws-cdk-lib.aws_s3'].targets['{{lang}}'].module, 'RootModule::S3');
